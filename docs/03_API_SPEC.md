...
- `inventory_probe: true` marks reduced exploration of inventory still under observation. Since 0.5.34 its target is the operator's `min_amount_sat` (falling back to the effective start/execution minimum when unset), not a fixed percentage. The execution floor, original batch/deficit ceiling and budget still apply. Existing `amount_sat` and economics fields describe the resulting amount.
...
- Since 0.5.34, with `amount_probe_adaptive` enabled, per-source and MPP-prepass route discovery can halve the queried amount after a no-route response on the same source. There are at most five discovery calls per attempt, including the initial query and a final execution-floor query, under the existing deadline. The source, exact target, exclusions and fee ceilings are preserved. Authentication, transport, timeout and payment-result errors do not trigger this fallback. Attempt history records the final queried/sent amount; manager logs show intermediate `no-route amount fallback` reductions. This is independent of inventory exploration sizing.

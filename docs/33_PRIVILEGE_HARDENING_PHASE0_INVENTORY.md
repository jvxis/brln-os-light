...
| LND upgrade — `internal/server/lnd_upgrade.go` | 2 | 1 | 0/0/0 | 1 | `upgrade.lnd`, `files.install_helper`, `service.status` | Version/checksum allowlist, staged binary verification, restart rollback, locked-wallet regression |
| Tor upgrade — `internal/server/tor_upgrade.go` | 3 | 1 | 0/0/0 | 1 | `packages.refresh_tor`, `upgrade.tor`, `files.install_helper` | Fixed repository/package set, lock timeout, signature/version checks, Tor health regression |
...
| `internal/server/assets/check-tor-update.sh` | Configures repository and installs/upgrades Tor packages | `upgrade.tor`; fixed repository/key/package set and package lock |

...
| Terminal credentials — `internal/server/terminal_status.go` | 0 | 0 | 0/0/0 | 0 | `terminal.credential.rotate`, `service.restart` | Migrated on 2026-08-13: password travels only in broker stdin and fixed `chpasswd` stdin, service user is independently resolved and must match, root is rejected, no staging file exists, and the secret-free Ubuntu 24.04 dry-run/injection/preservation gate passed |
...
| `internal/server/assets/lightningos-terminal-password.sh` | Bootstrap-only operator credential helper | Runtime use removed; installers retain the fixed local source for initial account setup, while rotations use `terminal.credential.rotate` without a staging file |

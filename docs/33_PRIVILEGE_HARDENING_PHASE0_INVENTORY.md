...
| --- | ---: | ---: | --- | ---: | --- | --- |
| System boundary — `internal/system/system.go` | 0 | 0 | 0/0/0 | 0 | `service.restart`; `host.power` with `reboot|poweroff` enum | Migrated on 2026-08-13: enforce-only typed calls, fixed delayed power command, strict response, and no direct systemctl/sudo/systemd-run fallback; executable names, arguments, shell syntax, unknown actions, disabled, and shadow modes fail closed |
...
| General handlers — `internal/server/handlers.go` | 0 | 0 | 0/0/0 | 0 | `service.restart`, `host.power`, `storage.lnd.permissions.repair` | Migrated: LND permissions use a fixed descriptor-relative operation; service and power handlers use enforce-only typed broker calls and retain the existing fixed service/action UI enums |
| Deleted generic transient unit — former `internal/server/systemd_run.go` | 0 | 0 | 0/0/0 | 0 | Removed after all runtime consumers migrated | Deleted on 2026-08-13; the permanent boundary test rejects any reintroduced generic privileged shell or transient-unit wrapper |
...
| System integrations — `internal/server/system_integrations.go` | 0 | 0 | 0/0/0 | 0 | `packages.feature.ensure(mdns)`, `files.system-integration.install`, `system.integrations.status`, `system.integrations.apply`, `app.bitcoincore.storage.ensure`, `system.integrations.finalize`, `service.restart` | Migrated on 2026-08-13: four fixed SHA-256 assets and destinations, root owner/mode checks, broker-validated marker/status, fixed TLS/mDNS/firewall helpers, idempotent LND policy, separate legacy Bitcoin storage enrollment without restart, and marker-last finalization; Linux/root plus service-user dry-run/injection/preservation gate passed |
| Terminal credentials — `internal/server/terminal_status.go` | 0 | 0 | 0/0/0 | 0 | `terminal.credential.rotate`, `service.restart` | Hardened on 2026-08-14: terminal disabled/read-only by default, dedicated secret-minimal runtime environment, locked Linux password, zero privileged supplementary groups, credential rotation updates only the GoTTY credential, service user is independently resolved and must match, and root is rejected |
...
| --- | --- | --- |
| `install.sh`, `install_existing.sh`, `install_existing_pi.sh` | Users/groups, packages, repositories, binaries, systemd, sudoers, UFW, ownership/modes, TLS, Postgres and application roots | Keep root-run bootstrap transactional; install broker first; validate generated units/sudoers; remove legacy grants only at T4. Go 1.24.12 and GoTTY 1.8.0 artifacts are now version/architecture/name/hash pinned and verified before inspection or extraction across all three variants; LND and repository setup authenticity remain open |
| `internal/server/assets/upgrade-app.sh` | Replaces manager/UI/assets and refreshes the cutover boundary | Broker-digest-pinned `upgrade.lightningos.start`; fixed repository and exact release tag/version/full-commit binding, locked UI dependencies, rollback bundle, no direct helper sudoers grant; publisher signature still required |
...
| `internal/server/assets/lightningos-manager-firewall.sh` | Writes/reconciles UFW policy | Installed only from a broker-pinned SHA-256 asset and executed from its fixed destination by `system.integrations.apply`; caller cannot select CIDR, path, port or argv |
| `internal/server/assets/setup-manager-tls-mdns.sh` | Writes TLS and mDNS policy | Installed only from a broker-pinned SHA-256 asset and executed with fixed environment/destination by `system.integrations.apply`; final disposable real-application matrix remains |
| Deleted legacy `internal/server/assets/reconcile-system-integrations.sh` | Former generic root reconciler | Removed on 2026-08-13; package, asset, policy, Bitcoin enrollment, finalization and restart stages are independently typed, locked and audited |
| Deleted legacy `internal/server/assets/lightningos-terminal-password.sh` | Former Linux-password helper for the web terminal | Removed on 2026-08-14; installers lock the dedicated account and remove privileged groups, while `terminal.credential.rotate` changes only the isolated GoTTY HTTP credential |

...
| System integrations — `internal/server/system_integrations.go` | 0 | 1 | 0/0/0 | 0 | `packages.feature.ensure(mdns)`, `files.reconcile_integrations` | The closed `mdns` package prerequisite is complete (`avahi-daemon` + `libnss-mdns`, Ubuntu 24/26, injection/preservation gate); fixed embedded assets/destinations, TLS/mDNS/firewall reconciliation, owner/mode checks, idempotency and rollback remain |

...
## Final catalog runtime reconciliation

The 2026-08-14 reconciliation distinguishes the already-complete typed
manager/broker boundary from container-level runtime confinement. CPU Miner,
which was the first broker migration model, now also runs as UID/GID 65534 with
a read-only root filesystem, all Linux capabilities dropped,
`no-new-privileges`, and a bounded `/tmp` tmpfs. Both current CPU images are
immutable registry digests. The two exact 0.5.2 tag references are recognized
only by the compatibility reader and are rewritten to the corresponding digest
before start; the broker never executes a mutable reference.

Two exhaustive source gates prevent another app from being omitted:

- all 20 registered apps must be classified as Compose, native systemd, or an
  in-manager feature without a separate privileged runtime;
- every service generated by all 13 Compose catalogs must have an explicit
  security profile. Unclassified services, `privileged`, host PID/IPC, device
  access, Docker-socket mounts, mutable `latest` images, or undeclared host
  networking fail the gate.

This policy does not impose one runtime shape blindly. Known-working behavior
from before PR #33 is preserved where the app has not yet proven a stricter
shape through install/start/stop/network/persistence tests. The explicit
compatibility exceptions are Bitcoin Core's root storage guard/official
entrypoint bootstrap, the upstream BTCPay and RoboSats stacks, PostgreSQL
bootstrap and writable startup paths in LNDg, and Tapd host networking. Tapd
still has a read-only root filesystem, no capabilities, and
`no-new-privileges`; its host network is required for local LND/LightningOS
connectivity and is the only admitted `network_mode: host` service.

LOS TESTE2 accepted commit `fe3f9dd7` from a clean trusted checkout. The CPU
Miner's baseline and fast images passed isolated, volume-free probes as the
declared non-root user with network disabled. The installed app then passed API
stop/start and effective Docker inspection with its prior bridge network,
configuration and final running state preserved. LND, Docker and PostgreSQL
activation identities and the running Bitcoin container were unchanged.
Evidence:

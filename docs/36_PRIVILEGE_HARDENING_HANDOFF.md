...
The BR⚡LN Community test app (app release 0.1.19 in the 0.5.33 series; first
...
password read. The chat (`ghcr.io/jvxis/brln-community-web:0.1.19`), the NIP-46
signer (`ghcr.io/jvxis/brln-signer:0.1.19`) and the Caddy proxy shared with Bark
are manifest-digest pinned; the broker probes nginx and Caddy and requires the
exact `brln-signer 0.1.19` version output before lifecycle execution. All
...
Since 0.1.19 the node also runs its own NIP-46 pairing relay (`pairing`). It runs

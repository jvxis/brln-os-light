...
The BR⚡LN Community test app (app release 0.1.23 in the 0.5.33 series; first
...
password read. The chat (`ghcr.io/jvxis/brln-community-web:0.1.23`), the NIP-46
signer (`ghcr.io/jvxis/brln-signer:0.1.23`) and the Caddy proxy shared with Bark
are manifest-digest pinned; the broker probes nginx and Caddy and requires the
exact `brln-signer 0.1.23` version output before lifecycle execution. All

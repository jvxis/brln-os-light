...
alter table reports_daily add column if not exists keysend_sent_sats bigint not null default 0;
alter table reports_daily add column if not exists keysend_sent_msat bigint not null default 0;
alter table reports_daily add column if not exists keysend_sent_count integer not null default 0;
...
// last complete day. An empty history has no gaps: the first persisted report
// establishes the start, so a new installation never invents earlier history.
...
func buildUpsertDaily(row Row) (string, []any) {
	...
	args := []any{
		...
		metrics.KeysendSentSat,
		metrics.KeysendSentMsat,
		metrics.KeysendSentCount,
...
on conflict (report_date) do update set
  ...
  keysend_sent_sats = excluded.keysend_sent_sats,
  keysend_sent_msat = excluded.keysend_sent_msat,
  keysend_sent_count = excluded.keysend_sent_count,
...
select
  ...
  coalesce(sum(keysend_sent_sats), 0),
  coalesce(sum(keysend_sent_msat), 0),
  coalesce(sum(keysend_sent_count), 0),
...
`, normalizeReportDate(startDate), normalizeReportDate(endDate)).Scan(
		...
		&totals.KeysendSentSat,
		&totals.KeysendSentMsat,
		&totals.KeysendSentCount,

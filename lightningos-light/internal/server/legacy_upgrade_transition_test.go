...
func TestLegacyTransitionReleaseIsLimitedToExactTargetRelease(t *testing.T) {
	...
	if err := validateLegacyTransitionRelease(valid, "0.5.15-Beta"); err != nil {
		t.Fatal(err)
	}
	for _, test := range []appReleaseInfo{
		{Version: "0.5.7-beta", Tag: "0.5.7-Beta", Commit: valid.Commit},
		{Version: valid.Version, Tag: "release/0.5.15-beta", Commit: valid.Commit},
		{Version: valid.Version, Tag: valid.Tag, Commit: "not-a-commit"},
	} {
		if err := validateLegacyTransitionRelease(test, "0.5.15-Beta"); err == nil {
...
func TestLegacyTransitionRootCommandVerifiesStagedHelper(t *testing.T) {
	...
	for _, expected := range []string{
		...
		"--version 0.5.15-beta --tag 0.5.15-Beta --commit " + info.Commit,
		"trap cleanup EXIT",
	} {
...
func TestLegacyTransitionRejectsUnsafeStagingInputs(t *testing.T) {
	info := appReleaseInfo{Version: "0.5.15-beta", Tag: "0.5.15-Beta", Commit: strings.Repeat("d", 40)}

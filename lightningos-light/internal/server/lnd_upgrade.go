...
func (s *Server) handleLNDUpgradeStatus(w http.ResponseWriter, r *http.Request) {
	...
	info, err := getLndReleaseInfo(ctx, force, currentVersion)
...
func getLndReleaseInfo(ctx context.Context, force bool, currentVersion string) (lndReleaseInfo, error) {
	cached, checkedAt, cachedCurrentVersion, ok := readLndReleaseCache()
	if ok && !force && lndReleaseCacheMatchesCurrent(cachedCurrentVersion, currentVersion) && time.Since(checkedAt) < lndReleaseCacheTTL {
		return cached, nil
	}

	info, err := fetchLatestLndRelease(ctx, currentVersion)
	if err != nil {
		if ok && lndReleaseCacheMatchesCurrent(cachedCurrentVersion, currentVersion) {
			return cached, err
		}
		return lndReleaseInfo{}, err
	}

	_ = writeLndReleaseCache(info, currentVersion)
	return info, nil
}

func fetchLatestLndRelease(ctx context.Context, currentVersion string) (lndReleaseInfo, error) {
	...
	return selectLndRelease(releases, currentVersion, time.Now().UTC())
}

func selectLndRelease(releases []ghRelease, currentVersion string, checkedAt time.Time) (lndReleaseInfo, error) {
	current := parseSemver(currentVersion)
	var selected lndReleaseInfo
	var selectedVersion parsedSemver
	for _, rel := range releases {
		info, ok := lndReleaseInfoFromGitHub(rel, checkedAt)
		if !ok {
			continue
		}
		candidate := parseSemver(info.Version)
		if !candidate.Valid {
			continue
		}
		if current.Valid {
		...
		if selected.Version == "" || compareSemver(selectedVersion, candidate) < 0 {
			selected = info
...
func lndReleaseInfoFromGitHub(rel ghRelease, checkedAt time.Time) (lndReleaseInfo, bool) {
	version := normalizeLndVersion(rel.TagName)
	if version == "" || !lndVersionPattern.MatchString(version) {
	...
	return lndReleaseInfo{
		...
		Channel:     lndReleaseChannel(rel.Prerelease),
		ReleasePage: rel.HtmlURL,
		CheckedAt:   checkedAt.Format(time.RFC3339),
	}, true
}

func lndReleaseChannel(prerelease bool) string {
	if prerelease {
		return "beta"
	}
	return "stable"
...
func normalizeLndVersion(value string) string {
	return strings.TrimSpace(strings.TrimPrefix(strings.TrimSpace(value), "v"))
}

func lndReleaseCacheMatchesCurrent(cachedCurrentVersion string, currentVersion string) bool {
	current := normalizeLndVersion(currentVersion)
	cachedCurrent := normalizeLndVersion(cachedCurrentVersion)
	if current == "" {
		return true
	}
	return cachedCurrent != "" && cachedCurrent == current
}

func readLndReleaseCache() (lndReleaseInfo, time.Time, string, bool) {
	...
	return cache.Info, checkedAt, cache.CurrentVersion, true
}

func writeLndReleaseCache(info lndReleaseInfo, currentVersion string) error {
	if info.CheckedAt == "" {
		info.CheckedAt = time.Now().UTC().Format(time.RFC3339)
	}
	cache := lndReleaseCache{
		CheckedAt:      info.CheckedAt,
		CurrentVersion: normalizeLndVersion(currentVersion),
		Info:           info,
	}

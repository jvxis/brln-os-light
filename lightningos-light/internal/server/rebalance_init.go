...
func (s *Server) initRebalance() {
	...
	ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
	defer cancel()
	intentSvc := s.automationIntents
	if intentSvc == nil {
		intentSvc = NewAutomationIntentService(pool, s.logger)
	}
	if err := intentSvc.EnsureSchema(ctx); err != nil {
		s.rebalanceErr = fmt.Sprintf("rebalance unavailable: failed to init automation intent schema: %v", err)
		s.logger.Printf("%s", s.rebalanceErr)
		return
	}
	s.automationIntents = intentSvc
	svc.automationIntents = intentSvc
	intentSvc.Prune(ctx)

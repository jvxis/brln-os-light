...
export type MeshPeer = { correlated_replies?: boolean; last_response?: string; node: number; name: string; paired: boolean; allow_relay: boolean; fingerprint: string }
...
export type MeshStatus = {
  ...
  history: { can_retry?: boolean; request_id?: string; response_id?: string; updated?: string; send_attempts?: number; bridge_accepted?: number; last_error?: string; operation?: string; id: string; peer: number; direction: string; state: string; txid: string; received: number; total: number; created: string }[]
  protocol_version: number
}
export type MeshAction = { request_id?: string; action: 'retry' | 'disconnect' | 'pair_probe' | 'pair_invite' | 'pair_accept' | 'pair_confirm' | 'pair_cancel' | 'peer_permissions' | 'install' | 'mode' | 'peer' | 'remove_peer' | 'preview' | 'send' | 'invoice' | 'request' | 'request_invoice' | 'pay' | 'cancel'; code?: string; device?: string; mode?: string; node?: number; name?: string; key?: string; allow_relay?: boolean; id?: string; raw_tx?: string; address?: string; amount_sat?: number; sat_per_vbyte?: number; invoice?: string; memo?: string; max_fee_sat?: number; confirm?: boolean; confirm_password?: string }
...
export const boostPeers = (payload?: { limit?: number; permanent?: boolean }) =>

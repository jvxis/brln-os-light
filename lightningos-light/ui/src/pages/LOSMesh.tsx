...
export default function LOSMesh() {
  ...
  return <section className="space-y-6">
    ...
    <div className="section-card space-y-4">
      <div role="tablist" aria-label={text('Rede de pagamento', 'Payment network')} className="flex gap-5 border-b border-white/15">{['onchain', 'lightning'].map(t => <button role="tab" aria-selected={tab === t} key={t} className={`border-b-2 px-2 py-3 text-sm ${tab === t ? 'border-emerald-400 text-emerald-300' : 'border-transparent text-fog/65'}`} onClick={() => setTab(t)}>{t === 'onchain' ? 'Bitcoin on-chain' : 'Lightning'}</button>)}</div>

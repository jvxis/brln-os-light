...
export default function LOSMesh() {
  ...
  const [lightningIntent, setLightningIntent] = useState<'pay' | 'receive'>('receive')
  const [incomingNotice, setIncomingNotice] = useState(0)
  const pendingSnapshot = useRef<string | null>(null)
  const requestListRef = useRef<HTMLDivElement>(null)
  const paymentFormRef = useRef<HTMLDivElement>(null)
  useEffect(() => {
    if (!status) return
    const snapshot = JSON.stringify(status.pending)
    if (pendingSnapshot.current !== null && snapshot !== pendingSnapshot.current && status.pending.length > 0) setIncomingNotice(Date.now())
    pendingSnapshot.current = snapshot
  }, [status])
  useEffect(() => {
    if (!incomingNotice) return
    const timer = window.setTimeout(() => setIncomingNotice(0), 12000)
  ...
  const tabClass = (selected: boolean) => `border-b-2 px-3 py-3 text-sm font-semibold focus-visible:outline focus-visible:outline-2 focus-visible:outline-emerald-400 ${selected ? 'border-emerald-400 text-emerald-300' : 'border-transparent text-fog/65 hover:text-fog'}`
  ...
  return <section className="space-y-6">
    ...
    <div className="section-card space-y-4">
      ...
      <div ref={nodeListRef} role="region" aria-label={text('Nós conhecidos pelo rádio', 'Nodes known to the radio')} tabIndex={0} className="max-h-[28rem] overflow-y-auto overscroll-contain space-y-3 pr-2">{filteredNodes.slice(currentNodePage * nodePageSize, (currentNodePage + 1) * nodePageSize).map(n => {
        ...
        return <div key={n.node} className="rounded-xl border border-white/10 p-3 space-y-2 text-sm"><p className="font-semibold">{n.name || n.short_name || `!${n.node.toString(16)}`}{n.name && n.short_name && n.name !== n.short_name ? ` (${n.short_name})` : ''} <span className="font-mono text-xs text-fog/60">!{n.node.toString(16).padStart(8,'0')}</span></p><p className="text-xs text-fog/60">{text('Visto pelo rádio', 'Last heard by radio')}: {n.last_heard ? new Date(n.last_heard*1000).toLocaleString() : '—'}{n.via_mqtt ? ' · MQTT' : ''}</p>
          <p>{pairedNode ? text('Contato verificado', 'Verified contact') : available ? text('LOS Mesh respondeu ao teste; identidade ainda não verificada.', 'LOS Mesh answered the check; identity not yet verified.') : text('Primeiro verifique a conexão. Após a resposta, adicione o contato.', 'Check the connection first. Once it responds, add the contact.')}</p>
          {!pairedNode && <div className="flex flex-wrap gap-2"><button className={available ? 'btn-secondary' : 'btn-primary'} disabled={busy || !connected || pendingNode} onClick={() => void act({action:'pair_probe',node:n.node})}>{text('1. Verificar conexão', '1. Check connection')}</button><button className={available ? 'btn-primary' : 'btn-secondary'} disabled={busy || !connected || !available || pendingNode} onClick={() => void act({action:'pair_invite',node:n.node})}>{text('2. Adicionar contato', '2. Add contact')}</button></div>}
        </div>
      })}</div>
    ...
    <div ref={requestListRef} className="section-card space-y-4 scroll-mt-20"><h3 className="text-lg font-semibold">{text('Solicitações recebidas', 'Received requests')}</h3>
    ...
    <div ref={paymentFormRef} className="section-card space-y-4 scroll-mt-20">
      ...
      {tab === 'onchain' ? <>
        ...
        {onchainIntent === 'receive' ? <>
          ...
          {!busy && (!canSend || !address.trim() || !Number.isSafeInteger(Number(amount)) || Number(amount) <= 0) && <p className="text-xs text-amber-200">{!connected ? text('Conecte o rádio para enviar a solicitação.', 'Connect the radio to send the request.') : status?.mode === 'relay' ? text('Aplique o modo Enviar e oferecer relay na aba Rádio para iniciar solicitações.', 'Apply Send and offer relay in the Radio tab to initiate requests.') : !peer ? text('Selecione um contato pareado.', 'Select a paired contact.') : !address.trim() ? text('Gere ou informe seu endereço de recebimento.', 'Generate or enter your receiving address.') : text('Informe um valor inteiro maior que zero em sats.', 'Enter a positive whole amount in sats.')}</p>}
        </> : <button className="btn-primary" disabled={busy || !canSend || Boolean(preview)} onClick={() => void review()}>{text('Revisar envio da minha carteira', 'Review sending from my wallet')}</button>}
    ...
    <div className="section-card space-y-4"><h3 className="text-lg font-semibold">{text('Histórico de sessões', 'Session history')}</h3><p className="text-xs text-fog/60">{text('Até 100 sessões recentes; retenção de 30 dias. Sem transações brutas, invoices completas ou preimages no histórico.', 'Up to 100 recent sessions; 30-day retention. No raw transactions, full invoices or preimages in history.')}</p>
      ...
      {status?.history.map(h => <div key={h.id} className="space-y-2 rounded-xl border border-white/10 p-3 text-sm"><p className="font-semibold">{h.direction === 'in' ? '↓' : '↑'} {h.state === 'awaiting_approval' && h.direction === 'out' ? text('Entregue ao contato — aguarda decisão no destino', 'Delivered to contact — awaiting decision at destination') : states[h.state] || h.state}</p><p className="text-xs text-fog/65">{new Date(h.created).toLocaleString()} · {h.received}/{h.total} {text('pacotes', 'packets')} · !{h.peer.toString(16).padStart(8, '0')}</p>{h.txid && <a className="block break-all font-mono text-xs text-emerald-300 underline" href={`https://mempool.space/tx/${h.txid}`} target="_blank" rel="noreferrer">{h.txid} ↗</a>}{['sending', 'receiving', 'awaiting_result'].includes(h.state) && <button className="btn-secondary" disabled={busy} onClick={() => void act({ action: 'cancel', id: h.id })}>{text('Cancelar envio', 'Cancel transfer')}</button>}</div>)}
    </div>
    </div>
    </div>

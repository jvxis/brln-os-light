...
export default function LOSMesh() {
  ...
  return <section className="space-y-6">
    ...
    <div className="section-card space-y-4">
      ...
      {tab === 'onchain' ? <>
        <div className="flex flex-wrap gap-3" role="group" aria-label={text('Operação Bitcoin', 'Bitcoin operation')}>
          {(['receive', 'send'] as const).map(intent => <button key={intent} aria-pressed={onchainIntent === intent} className={onchainIntent === intent ? 'btn-primary' : 'btn-secondary'} disabled={busy || Boolean(preview)} onClick={() => { if (intent !== onchainIntent) { setOnchainIntent(intent); setAddress(''); setAmount(''); setRaw(''); setError(''); setNotice('') } }}>{intent === 'receive' ? text('Quero receber', 'Receive bitcoin') : text('Enviar bitcoin', 'Send bitcoin')}</button>)}
        </div>

...
function StatusBadge({ status, t }: { status: string; t: any }) {
  ...
  const color = ['completed', 'succeeded', 'success'].includes(normalized)
    ? 'border-emerald-400/30 bg-emerald-500/15 text-emerald-200'
    : ['failed', 'cancelled'].includes(normalized)
      ? 'border-rose-400/30 bg-rose-500/15 text-rose-200'
      : normalized === 'paused'
        ? 'border-sky-400/30 bg-sky-500/15 text-sky-200'
        : normalized === 'waiting_liquidity'
          ? 'border-amber-400/30 bg-amber-500/15 text-amber-200'
          : 'border-brass/30 bg-brass/10 text-brass'
  return <span className={`shrink-0 rounded-full border px-2.5 py-1 text-[10px] uppercase tracking-wide ${color}`}>{t(`loopOutBrln.status.${normalized}`, { defaultValue: normalized.replace(/_/g, ' ') })}</span>
}
